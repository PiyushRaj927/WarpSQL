...
echo $scriptDir/extensions_test
# Find all test script files in the directory
test_scripts=$(find $scriptDir/extensions_test -type f -name 'test_*.sh')
...
IFS=',' read -ra EXTENSION_LIST <<< "$1"
    for extension in "${EXTENSION_LIST[@]}"; do
        test_script="$scriptDir/extensions_test/test_${extension}.sh"
